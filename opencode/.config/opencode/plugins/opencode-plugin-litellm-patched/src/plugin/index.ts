 ...
 */
async function backgroundRefresh(baseURL: string, quiet: boolean = false): Promise<void> {
  ...
  try {
    const built = await withTimeout(
      discoverModels(baseURL, ctx.apiKey, ctx.customHeaders, ctx.providerId, quiet),
  ...
  return {
    config: async (config: any) => {
      ...
      for (const { id: providerId, provider } of liteLLMProviders) {
        ...
        */
        ...
        // Quiet mode: suppress all console.log/warn output from this plugin.
        // Can be enabled via provider options: { "options": { "quiet": true } }
        // or globally via the OPENCODE_LITELLM_QUIET=1 environment variable.
        const quiet =
          options.quiet === true ||
          process.env.OPENCODE_LITELLM_QUIET === '1'
        ...
        */
        ...
        refreshContexts.set(baseURL, { apiKey, customHeaders, providerId, quiet })
        ...
        const built = await withTimeout(
          discoverModels(baseURL, apiKey, customHeaders, providerId, quiet),
          DISCOVERY_TIMEOUT_MS,
        )
        if (built && Object.keys(built).length > 0) {
    ...
    event: async ({ event }) => {
      ...
      for (const baseURL of refreshContexts.keys()) {
        const ctx = refreshContexts.get(baseURL)
        void backgroundRefresh(baseURL, ctx?.quiet)
      }
    },
  }
}
